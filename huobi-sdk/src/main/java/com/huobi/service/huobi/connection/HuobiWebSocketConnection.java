...
public class HuobiWebSocketConnection extends WebSocketListener implements WebSocketConnection {
  ...
  void connect() {
    if (state == ConnectionStateEnum.CONNECTED) {
      log.info("[Connection][" + this.getId() + "] HUOBI Already connected");
      return;
    }
    log.info("[Connection][" + this.getId() + "] HUOBI Connecting...");
    webSocket = ConnectionFactory.createWebSocket(okhttpRequest, this);
  }

  public void reConnect(int delayInSecond) {
    log.warn("[Sub][" + this.getId() + "] HUOBI Reconnecting after "
  ...
  public void send(String str) {
    ...
    if (!result) {
      log.error("[Connection Send][" + this.getId() + "] HUOBI Failed to send message");
      closeOnError();
    }
  }
  ...
  public void onMessage(WebSocket webSocket, String text) {
    ...
    log.info("【HUOBI】[On Message Text]:{}", text);
    try {
      ...
      log.error("[On Message][{}]: HUOBI catch exception:", this.getId(), e);
      closeOnError();
    }
  }
  ...
  public void onMessage(WebSocket webSocket, ByteString bytes) {
    ...
    try {
      ...
      try {
        ...
        log.error("[Connection On Message][" + this.getId() + "] HUOBI Receive message error: " + e.getMessage());
        closeOnError();
        return;
      }
      log.debug("HUOBI [Connection On Message][{}] {}", this.getId(), data);
      ...
      if (jsonObject.containsKey("status") && !"ok".equals(jsonObject.getString("status"))) {
        ...
        log.error("HUOBI [Connection On Message][" + this.getId() + "] Got error from server: " + errorCode + "; " + errorMsg);
        close();
      ...
      log.error("HUOBI [Connection On Message][" + this.getId() + "] Unexpected error: " + e.getMessage());
      closeOnError();
    }
  }

  private void onError(String errorMessage, Throwable e) {
    log.error("HUOBI [Connection error][" + this.getId() + "] " + errorMessage);
    closeOnError();
  }
  ...
  private void onReceive(JSONObject jsonObject) {
    ...
    try {
      ...
      onError("HUOBI Process error: " + e.getMessage() + " You should capture the exception in your error handler", e);
      return;
    }
  ...
  public void close() {
    log.error("HUOBI [Connection close][" + this.getId() + "] Closing normally");
    webSocket.cancel();
    webSocket = null;
  ...
  public void onOpen(WebSocket webSocket, Response response) {
    ...
    log.info("HUOBI [Connection][" + this.getId() + "] Connected to server");
    ...
    if (authNeed) {
      if (AUTH_VERSION_V1.equals(this.getAuthVersion())) {
        ...
        onError("HUOBI Unsupport signature version: " + this.getAuthVersion(), null);
        close();
        return;
      }
  ...
  public void onFailure(WebSocket webSocket, Throwable t, Response response) {
    onError("HUOBI Unexpected error: " + t.getMessage(), t);
    closeOnError();
  }
  ...
  private void closeOnError() {
    if (webSocket != null) {
      ...
      log.error("HUOBI [Connection error][" + this.getId() + "] Connection is closing due to error");
    }
  }
  ...
  private void sendAuthV2_1() {
    ...
    try {
      ...
      onError("HUOBI Unexpected error when create the signature v2: " + e.getMessage(), e);
      close();
      return;
    }
  ...
  private void sendAuthV2() {
    ...
    try {
      ...
      onError("HUOBI Unexpected error when create the signature: " + e.getMessage(), e);
      close();
      return;
    }
