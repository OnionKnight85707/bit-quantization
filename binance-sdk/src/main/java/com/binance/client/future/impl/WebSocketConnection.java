...
public class WebSocketConnection extends WebSocketListener {
    ...
    private String subscriptionUrl = FutureBinanceApiConstants.WS_API_BASE_URL;
    ...
    WebSocketConnection(String apiKey, String secretKey, SubscriptionOptions options, WebsocketRequest request,
        ...
        log.info("Binance [Sub] Connection [id: " + this.connectionId + "] created for " + request.name);
    }
    ...
    void connect() {
        if (state == ConnectionState.CONNECTED) {
            log.info("[Sub][" + this.connectionId + "] Binance Already connected");
            return;
        }
        log.info("[Sub][" + this.connectionId + "] Binance Connecting...");
        webSocket = RestApiInvoker.createWebSocket(okhttpRequest, this);
    }

    void reConnect(int delayInSecond) {
        log.warn("[Sub][" + this.connectionId + "] 【Binance】 Reconnecting after " + delayInSecond + " seconds later");
        if (webSocket != null) {
            webSocket.cancel();
            webSocket = null;
        }
    ...
    void send(String str) {
        ...
        if (!result) {
            log.error("[Sub][" + this.connectionId + "] Binance Failed to send message");
            closeOnError();
        }else
        {
            //todo: fix bug
    ...
    public void onMessage(WebSocket webSocket, String text) {
        ...
        log.debug("【Binance】 [On Message]:{}", text);
        try {
            ...
            log.error("【Binance】[On Message][{}]: Binance catch exception:", connectionId, e);
            closeOnError();
        }
    }
    ...
    private void onError(String errorMessage, Throwable e) {
        ...
        log.error("[Sub][" + this.connectionId + "] Binance " + errorMessage);
    }
    ...
    private void onReceive(JsonWrapper jsonWrapper) {
        ...
        try {
            ...
            onError("Binance Failed to parse server's response: " + e.getMessage(), e);
        }
        try {
            request.updateCallback.onReceive(obj);
        } catch (Exception e) {
            onError("【Binance】 Process error: " + e.getMessage() + " You should capture the exception in your error handler", e);
        }
    }
    ...
    public void close() {
        log.error("[Sub][" + this.connectionId + "] Binance Closing normally");
        webSocket.cancel();
        webSocket = null;
    ...
    public void onOpen(WebSocket webSocket, Response response) {
        ...
        log.info("[Sub][" + this.connectionId + "] Binance Connected to server");
    ...
    public void onFailure(WebSocket webSocket, Throwable t, Response response) {
        onError("com.binance.client.future.impl.WebSocketConnection  onFailure ,t="+t+"response="+response , t);
        closeOnError();
    }
    ...
    private void closeOnError() {
        if (webSocket != null) {
            ...
            log.error("[Sub][" + this.connectionId + "] Binance Connection is closing due to error");
        }
    }
}
