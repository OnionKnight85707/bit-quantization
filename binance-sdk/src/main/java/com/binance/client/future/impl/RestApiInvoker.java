...
abstract class RestApiInvoker {
 ...
 //   private static final OkHttpClient client = new OkHttpClient().newBuilder().pingInterval(30,TimeUnit.SECONDS).build();
 private static final OkHttpClient client = ProxyUtils.getClient();
    ...
    static <T> T callSync(RestApiRequest<T> request) {
        try {
            ...
            if (str.contains("executedQty"))
                log.info("binance RestApiInvoker : Response =====> " + str);
